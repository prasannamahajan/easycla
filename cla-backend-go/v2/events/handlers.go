	...
	api.EventsGetFoundationEventsAsCSVHandler = events.GetFoundationEventsAsCSVHandlerFunc(
		func(params events.GetFoundationEventsAsCSVParams, authUser *auth.User) middleware.Responder {
			...
			if !utils.IsUserAuthorizedForProject(authUser, params.FoundationSFID) {
				return Response(http.StatusForbidden, runtime.JSONMime, runtime.JSONProducer(), &models.ErrorResponse{
					Code: "403",
			...
			if err != nil {
				return Response(http.StatusBadRequest, runtime.JSONMime, runtime.JSONProducer(), errorResponse(err))
			}
		...
		func(params events.GetProjectEventsAsCSVParams, authUser *auth.User) middleware.Responder {
			...
			if err != nil {
				if err == projects_cla_groups.ErrProjectNotAssociatedWithClaGroup {
					return Response(http.StatusNotFound, runtime.JSONMime, runtime.JSONProducer(), &models.ErrorResponse{
						Code: "404",
						Message: fmt.Sprintf("EasyCLA - 403 Forbidden - project %s not found in cla",
							params.ProjectSFID),
					})
				}
				return Response(http.StatusInternalServerError, runtime.JSONMime, runtime.JSONProducer(), errorResponse(err))
			}
...
func Response(httpStatus int, contentType string, contentProducer runtime.Producer, data interface{}) middleware.Responder {
	return middleware.ResponderFunc(func(rw http.ResponseWriter, pr runtime.Producer) {
		rw.Header().Set(runtime.HeaderContentType, contentType)
		rw.WriteHeader(httpStatus)
		contentProducer.Produce(rw, data)
	})
}
